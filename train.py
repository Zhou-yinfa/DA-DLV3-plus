...
from utils.voc_config import VOC_YEAR
...
if __name__ == "__main__":
    ...
    num_classes     = 6
    #-----------------------------------------------------#
    #   DA-DLV3+ 注意力模块配置（论文架构）
    #
    #   ASPP后注意力:
    #     use_dapsa  → DAPSA 方向感知极化自注意力
    #
    #   浅层注意力:
    #     use_ca     → 浅层CA坐标注意力，增强位置感知
    #
    #   核心改进:
    #     use_dcn    → 可变形卷积 (DeformASPP + DeformDecoder)
    #
    #   推荐配置:
    ...
    #------------------------------------------------------------------#
    #   dice_loss       是否使用dice_loss来改善模型
    #   建议选项：
    #   种类少（几类）时，设置为True
    #   种类多（十几类）时，如果batch_size比较大（10以上），那么设置为True
    #   种类多（十几类）时，如果batch_size比较小（10以下），那么设置为False
    #   注意：focal_loss=True + dice_loss=True 可能导致NaN，建议先用CE Loss单独训练
    #------------------------------------------------------------------#
    dice_loss       = True
    #------------------------------------------------------------------#
    #   是否使用focal loss来防止正负样本不平衡
    #   注意：gamma=2可能导致梯度爆炸，建议先用CE Loss训练稳定后再启用
    #------------------------------------------------------------------#
    focal_loss      = True
    #------------------------------------------------------------------#
    ...
    #cls_weights     = np.array([3, 1, 2, 1.5, 0.5, 0.8], np.float32)  # 类别权重：背景3倍、水系1倍、林地2倍、道路1.5倍、耕地0.5倍、建筑0.8倍
    #------------------------------------------------------------------#
    ...
    model   = DeepLab(num_classes=num_classes, backbone=backbone, downsample_factor=downsample_factor,
                       pretrained=pretrained, use_dcn=use_dcn,
                       use_dapsa=locals().get('use_dapsa', False),
                       use_cbam=locals().get('use_cbam', False),
                       use_ca=locals().get('use_ca', False))
    if not pretrained:
        weights_init(model)
    if model_path != '':
        #------------------------------------------------------#
    ...
    #   读取数据集对应的txt
    #---------------------------#
    with open(os.path.join(VOCdevkit_path, f"{VOC_YEAR}/ImageSets/Segmentation/train.txt"),"r") as f:
        train_lines = f.readlines()
    with open(os.path.join(VOCdevkit_path, f"{VOC_YEAR}/ImageSets/Segmentation/val.txt"),"r") as f:
